...
const EmailList = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Parse emails and update state
      const parsedEmails = parseCompanyInfo(data.generatedText);
...
function parseCompanyInfo(data: string): { sender: string; subject: string; preview: string }[] {
  return data.split('---').map((entry: string) => {
    const lines = entry.split('\n').filter(line => line.trim() !== '');
    console.log(lines)
    const senderLine = lines[0]
    const summaryLine = lines[1]
    const statusLine = lines[2]
    const sender = senderLine.replace('会社名: ', '').trim();
    const summary = summaryLine.replace('要約: ', '').trim();
    const status = statusLine.replace('選考ステータス: ', '').trim();
    console.log(sender,summary,status)
    return { sender, subject: status, preview: summary };
  })
}
